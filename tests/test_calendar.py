...
def test_the_six_observed_code_combinations(codes: list[int], service_id: str) -> None:
    ...
    assert Service.from_codes(codes).service_id == service_id
...
def test_duplicate_code_pairs_collapse_to_one_service() -> None:
    ...
    assert Service.from_codes([3, 4, 5]) == Service.from_codes([3, 6, 7])


def test_service_with_no_operating_days_is_rejected() -> None:
    with pytest.raises(ValueError, match="no days at all"):
        _ = Service.from_codes([3]).service_id  # only the low-floor flag


def test_weekday_flags_match_gtfs_column_order() -> None:
    assert Service.from_codes([2, 3]).weekday_flags == (1, 1, 1, 1, 1, 0, 0)
    assert Service.from_codes([3, 4]).weekday_flags == (0, 0, 0, 0, 0, 1, 0)
    assert Service.from_codes([3, 5]).weekday_flags == (0, 0, 0, 0, 0, 0, 1)
    assert Service.from_codes([2, 3, 4, 5]).weekday_flags == (1, 1, 1, 1, 1, 1, 1)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (2038, dt.date(2038, 4, 25)),  # latest possible date
    ],
)
def test_easter_sunday(year: int, expected: dt.date) -> None:
    assert easter_sunday(year) == expected


def test_easter_sunday_is_always_a_sunday() -> None:
    assert all(easter_sunday(y).weekday() == 6 for y in range(2020, 2100))
...
def test_holiday_takes_the_sunday_timetable() -> None:
    weekday = Service.from_codes([2, 3])
    sunday_only = Service.from_codes([3, 5])
...
def test_holiday_on_a_weekday_removes_and_adds_the_right_services() -> None:
    services = [Service.from_codes([2, 3]), Service.from_codes([3, 5])]
