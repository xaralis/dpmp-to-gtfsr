...
def stop_id(station: int, platform: int) -> str:
    ...
    The old API encoded the same thing as ``station * 100 + platform`` in a
    single number; this spells it out instead so the ids stay readable and
    unambiguous past 99 platforms.
    """
...
def trip_id(line: str, connection: int) -> str:
    ...
    ``connection`` is the upstream's own trip number, which ``/vehicles``
    reports directly as ``connectionId`` -- so realtime maps onto static
